...
export class Migration20220705161126 extends Migration {

  async up(): Promise<void> {
    this.addSql('create table `specialty` (`id` integer not null primary key autoincrement, `name` text not null);');

    this.addSql('create table `company` (`id` integer not null primary key autoincrement, `name` text not null, `logo_url` text not null);');
    ...
    this.addSql('create table `company_city` (`company_id` integer not null, `city_id` integer not null, constraint `company_city_company_id_foreign` foreign key(`company_id`) references `company`(`id`) on delete cascade on update cascade, constraint `company_city_city_id_foreign` foreign key(`city_id`) references `city`(`id`) on delete cascade on update cascade, primary key (`company_id`, `city_id`));');
    this.addSql('create index `company_city_company_id_index` on `company_city` (`company_id`);');
    this.addSql('create index `company_city_city_id_index` on `company_city` (`city_id`);');
  }
